...
def SNN():
    ...
    HiddenGroup1 = NeuronGroup(N, eqs, threshold='v> 0.8', reset='v = 0', refractory=5*ms)
    ...
    global H1toO
    H1toO = Synapses(HiddenGroup1, OutputGroup, 'w:1', on_pre='v_post += w')
    H1toO.connect()
    ...
    while True: 
        ...
        for j in range(0,NB_LIGNES-1):
            for k in range(0,NB_COLONNES-1):
                ...
                if mode == LEARNING:
                    if j == 10 and k == 10:
                        print "Poids actuel: " + str(ItoH1.w[noNeurone])
                        print "noLearning: " + str(learningNo)
                    # Il faut aller chercher la somme
                    ItoH1.w[noNeurone] = (ItoH1.w[noNeurone] + intensite(j,k)) / learningNo   
                    if j == 10 and k == 10:
                        print "Intensite lue: " + str(intensite(j,k))
                        print "poids du synapse: " + str(ItoH1.w[noNeurone])
                if mode == TEST:
